...
> 说明：
> - `is` 模式匹配**仅**用于 `while` 循环体内，不可独立使用（2.0 起 IR 前端对循环外的 `is` 报错，PB-27.1；A 后端 `parser.c` 仍按 `if` 展开，属已知残留）。循环条件表达式的值赋给隐式变量 `__is_val`（类型感知，等于条件类型，PB-27.7），`is` 对其进行匹配。`do` 不支持 `is`（PB-27.1），因为 `do` 先执行块再判断条件，`__is_val` 的语义容易产生混乱。
