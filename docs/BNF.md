...
> - 版本：v2.3（2026-09-19 修订：`?=` 安全赋值记号从语法移除——`=` 已无条件执行 §12.1 可见性检查，`TOK_SAFE_ASSIGN` 仅词法保留，PA-20）。上一版 v2.2（2026-09-19，`is <pattern> => <statement>` 单语句形式移除——`=>` 保留词法但语法不使用，`is` 只保留块形式且仅配合 `while` 循环体，PA-13 / PB-27.6 双前端对齐）。更早 v2.1（2026-09-15，指针语法收敛：一元 `*` 解引用移除，解引用统一 `.()`/`.(T)`/`->`）。
