...
> 说明：
> - `is` 模式匹配**仅**用于 `while` 循环体内，不可独立使用（双前端均在循环体外直接报错拒绝：1.0 线 v1.0.2 起 A 后端 `parser.c` 加 `while_depth` 守卫（PA-18）、IR 前端 `irparse.c` 同；2.0 线 IR 前端先落地（PB-27.1），A 后端守卫待回灌）。循环条件表达式的值赋给隐式变量 `__is_val`（类型感知，等于条件类型，PB-27.7），`is` 对其进行匹配。`do` 不支持 `is`（PB-27.1），因为 `do` 先执行块再判断条件，`__is_val` 的语义容易产生混乱。
