...
| ---- | ---- | :---: | :---: |
| 语句 | if/else、while、do、for、switch/case/default、break/continue | ✅ | ✅ |
| 语句 | goto/label（独立编号空间）、is 块形式 + **`=>` 单语句（8/30）** | ✅ | ✅ |
| 语句 | return（含 `return {…}` sret 聚合）、cooking 块、block | ✅ | ✅ |
| 声明 | 变量（窄类型 i8..u32/f32 截断语义）、可见性 const/static/flow/var | ✅ | ✅ |
| 声明 | 数组（静态/动态 `[N...]`/初始化/嵌套初始化）、多变量 `var {a,b}` | ✅ | ✅ |
| 声明 | struct/union/enum（命名/嵌套/整体赋值/位域/初始化列表） | ✅ | ✅ |
| 声明 | 函数指针、类型别名、字符串/char[]、use/link（单文件模型跳过） | ✅ | ✅ |
| 表达式 | 算术/位/比较/逻辑短路、复合赋值、前缀/后缀 ++/-- | ✅ | ✅ |
| 表达式 | 下标/切片（`[lo..hi]` 含 len）、成员访问 `.`（链式） | ✅ | ✅ |
| 表达式 | 解引用 `*`/`&`/`.()`、sizeof/alignof/offsetof/visof/malloc | ✅ | ✅ |
| 表达式 | 嵌套/间接调用（IR_CALLI）、编译期函数 cooking-call | ✅ | ✅ |
| 表达式 | **`->` 指针成员访问**（8/30：pt[] 表 + ir_arrow.nc） | ✅ | ✅ |
| 表达式 | **`?:` 三元条件运算符**（8/30：短路求值 + ir_expr.nc 四后端） | ✅ | ✅ |
| 编译期 | static_assert、编译期变量表（ct_vars） | ✅ | ✅ |
...
- IR 语法覆盖剩余（2026-08-30 补齐 `->` + `?:` 后）：**无**——语法全覆盖达成；命名空间无语言定义待议；`is =>` 单语句 8/30 完成
- **PB-24 统一管线决策（2026-08-30 定案：保留双路线，A=1.0 / B=2.0）**——IR 覆盖全量语法后决策条件满足，ltree 拍板：A 方案先做对外可用 **1.0**（产品化），B 方案作为下一代 **2.0** 持续演进（详见 `docs/VERSIONING_ROADMAP.md` + `docs/PB24_DECISION.md`）；main 为稳定发布线、PA 特性冻结、PB 持续开发
