...
**cooking 编译期（A 方案）**——顶层+函数内 `cooking { static_assert/const 变量 }`：独立常量折叠链
pc_*（int/一元/四则/移位/比较/位/逻辑/括号/enum/可见性枚举/sizeof）+ ct_vars 表 + 运行时折叠，
ir_cook.nc 升 IR_SUBSET（4 backends consistent）。
