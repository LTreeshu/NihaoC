...
**结论（PB-24 决策输入）**：IR 前端覆盖全量语法的差距已收敛到 **2 个表达式级特性**（`->` 指针成员访问、`?:` 三元），均为小工作量（各 ~30 行）；补齐后 B 方案即可宣称"语法全覆盖"，PB-24 统一管线决策具备完整输入。命名空间无语言定义，不构成覆盖缺口。
...
- IR 语法覆盖剩余（2026-08-30 盘点）：**`->` 指针成员访问、`?:` 三元**（2 个表达式级，见 §3.1）；命名空间无语言定义待议；`is =>` 单语句已 8/30 完成
