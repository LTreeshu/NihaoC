...
- [ ] **指针声明语法决策 + 实现**：A 方案 `p = &agg` 中 `p` 隐式 `int32_t` 导致 `p->x` 编译失败（ir_arrow.nc 因此归 IR_ONLY）。需定语言规则：`p = &x` 自动推断指针类型 vs 显式声明语法（如 `p Point* = &pt`）。**这是 1.0 唯一影响"语法宣称全覆盖"的硬缺口**（§3.1 表格 A 列 `->` 虽标 ✅，但无类型指针声明不可用）
...
- [ ] ir_arrow.nc 从 IR_ONLY 转正为 IR_SUBSET（四后端一致性）
...
1. **1.0 指针声明语法**：自动推断（`p = &pt` 推导 `Point*`）还是显式声明（`p Point*`）？（影响 BNF + A/B 双实现）
