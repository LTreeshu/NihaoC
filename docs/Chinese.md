...
> - **`->` 指针成员访问**：`p->field` 等价 `p.()` 解引用，支持链式 `p->a->b`（A 方案与 IR 层双线一致，2026-08-19 起）。
...
赋值时, **目标变量的存储期必须不短于源变量** (即目标寿命 ≥ 源寿命)，否则会导致悬垂指针。编译器依据下表进行静态检查：
...
- **安全**：赋值允许，编译器通过生命周期检查。
- **错误**：目标寿命可能短于源，禁止赋值(编译错误)。

例如，`flow` 指针不能赋给 `static` 指针，因为 `flow` 可能在函数结束时销毁，而 `static` 需要长期持有。`static` 可以赋给 `const`(只读借用)，但不能赋给 `var` 或 `flow`(避免可变借用或转移导致生命周期不匹配)。
...
### 12.1 核心传递规则(针对 `void` 指针)

对于拥有所有权的 `flow` 指针，其向其他属性的传递遵循以下规则(源状态变化)：
...
### 12.2 综合传递矩阵(存储期 + 所有权/借用)

将存储期兼容矩阵与上述规则合并，得到完整的指针传递行为表。每个单元格格式为：`允许/禁止 (源状态变化)`。
...
编译器对示例代码执行多阶段静态检查，确保所有指针传递均符合存储期兼容矩阵和所有权/借用规则。其分析流程如下：

1. **属性推导**：为每个变量和函数参数标注存储期属性（`const`/`static`/`flow`/`var`）及所有权状态（拥有 / 借用）。
2. **可见性兼容性检查**：依据第 11.2 节的存储期兼容矩阵，验证赋值或传递时目标寿命不低于源寿命。
...
| `const read_ref void = ptr`                         | 赋值（`_flow` → `_const`）    | 存储期兼容，`_const` 寿命长于 `_flow`（允许）；传递规则：`flow→const` 为**只读借用**。       | **`ptr` 继续冻结**（多个借用共存），`read_ref` 为只读借用（`_const`）。                              |
