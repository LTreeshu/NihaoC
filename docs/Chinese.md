...
- `len(x)` 逻辑长度（2026-08-19）：数组=容量（多维为各维乘积）；动态字符串 `char[]`=字面量长度；
...
> `len(x)` 自 BNF v2.6 起在 1.0 线由 A 后端提供上述三类取值：实参的逻辑长度静态不可知
> （标量、非字面量边界的切片等）时前端即时报错；`ir-*` 后端已实现三类取值但静态不可知时返回 0。
...
> 口径：`print` 由 1.0 线 A 后端提供，`ir-*` 后端（2.0 线）未内建 `print`（直出成未定义符号，
> 链接期报错），跨后端一致的用例只使用 `puts`。`puts` 两线均为 C 库直通。
...
> **数组指针与切片（2026-09-20 定案，A 方案 c/native 后端）**
>
...
> - **切片读 `p[a..b]`**：值语义是「指向第 a 号元素的指针」。赋给数组变量时按该数组的声明容量逐元素复制；写成类型推断声明 `s = p[a..b]` 时得到元素类型的指针视图（不复制），`s.()` 即第 a 号元素。上下界同为字面量时后端会记录 `b-a` 供 `len(s)` 取用（§2.3）。省略起点写作 `p[..b]`，等价 `p[0..b]`，长度记为 `b`。
...
> - **实现口径（2026-09-20，v1.0.2 / PA-31）**：上条的数组存储与两条诊断由 A 方案 c/native 后端提供；`ir-*` 后端既不检查容量也不拒绝非 `char` 元素，属 2.0 待对齐。
...
> **实现状态**：本表检查在 1.0 线（≤ v1.0.2）尚未强制执行，上表的非法接收目前只被规范禁止、不被编译器报错；返回值可见性前缀检查自 2.0 线（PB-29）起实现，1.0 冻结线不移植。对应关系见 [`IMPLEMENTATION_STATUS.md`](./IMPLEMENTATION_STATUS.md) 的「调用方接收规则（§7.3）」表。
...
对已声明的 `flow` 变量做**整变量重绑定**（`p = 右值`）时按新右值重新判定上述来源；`p.(T) = v`、`p[i] = v` 改的是所指对象，不改变 `p` 自身的来源。重绑定会放弃对旧堆块的所有权，而编译器不会在重绑定点插入释放（保守做法，避免别名双释放），旧堆块因此泄漏——精确回收属 2.0 的所有权转移分析。
...
> **实现状态**：1.0 线（≤ v1.0.2）解析参数属性前缀但不强制执行对应的借用语义——形参统一按 `var` 处理；参数前缀的所有权/借用检查自 2.0 线（PB-26）起实现。具体代码位置见 `IMPLEMENTATION_STATUS.md` 的「函数参数传递（§12.2）」表。
> 1.0 冻结线不移植该检查；2.0 线已实现并验证（PB-26）。
...
解引用 `.(T)` 自身即执行安全检查，无需附加记号（原 `?.` / `?(` 已从语法移除，见 BNF v2.3）：编译器先校验指针可见性（源被冻结或已失效即编译期报错），再在编译期比较 `sizeof(T)` 与该指针静态所指对象的字节数，读取宽度超出即报越界：
...
2.  **可见性检查的指针安全**：每一次赋值（`=` 与复合赋值）都会检查可见性兼容性，无需专用记号（原 `?=` 安全赋值已从语法移除，v2.3 / PA-20）
