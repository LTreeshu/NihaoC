...
| 规范要求 | 编译器实现 | 对应代码 | 状态 |
|---------|----------|---------|------|
| 裸标识符赋值检查 | 完整实现 | parser.c:1026–1033（声明初始化）、2387（表达式赋值） | ✅ 已实现 |
| 表达式级检查 | 仅检查裸标识符 | parser.c:1026–1033、2387 | ⚠️ 部分实现 |
...
| 规范要求 | 编译器实现 | 对应代码 | 状态 |
|---------|----------|---------|------|
| `is` 仅配合 `while`（块形式） | `parse_is_stmt` 只在 while 体内调用 | parser.c:1102–1157 | ✅ 已实现 |
| `do` 不支持 `is` | `TOK_DO` 走 `parse_statement` 通用体，不识别 is-clause | parser.c:1257+ | ✅ 已实现（隐式） |
| `is <int-literal>` / `is -<int>` | `== v` / `== -v` | parser.c:1110–1122 | ✅ 已实现 |
| `is lo..hi` 闭区间 | `>= lo && __is_val <= hi` | parser.c:1117–1127 | ✅ 已实现 |
| `is <visibility-enum>` | 比较 `NH_*` 常量 | parser.c:1129–1145 | ✅ 已实现 |
| `is <enum-variant>` / 已知常量 | `TOK_IDENTIFIER` 分支按值比较 `== pat` | parser.c:1129–1136 | ✅ 已实现 |
| `is _` 通配符恒匹配 | 落入"无效模式"分支，`_` 被当标识符输出到 C | parser.c:1146–1149 | ⚠️ 1.0 线未实现（2.0 已覆盖：PB parser.c:1201 + IR 侧 PB-27.5） |
| `is <identifier>` 变量绑定 | 按**值**比较，非绑定 | parser.c:1129–1136 | ⚠️ 语义差异（2.0 范围，PB-27 类型感知后统一） |
| `is <pat> => <stmt>` 单语句 | 已移除（BNF v2.2 / PA-13），双前端仅接受块形式 | parser.c:1152–1156、irparse.c:2136–2141 | ✅ 已按规范移除 |
| 反向范围 `lo > hi` 编译期校验 | 无 | parser.c:1117–1127 | ⚠️ 未实现（2.0 范围，PB-27.4 已覆盖） |
...
| `flow` 动态分配 + 自动释放 | 块退出时自动 free | parser.c:1394–1405 | ✅ 已实现 |
| `flow` 返回值所有权转移 | `ownership_transferred` 标志 | parser.c:1296–1306 | ✅ 已实现 |
