...
- **2026-09-03 1.0.x 指针语法收敛（`PA`）+ `v1.0.1` tag（commit `75c59cc`）**：移除 `T*` 显式声明与一元 `*` 解引用，解引用统一 `.()`/`.(T)`/`->`；另含 Linux 平台修复（PA-9）、`is` 模式匹配规范定案（仅文档）、§12 指针传递矩阵文档统一。
- **2026-09-19 `v1.0.2` 发布准备（待 tag）**：`PA` 上 v1.0.1 之后的 8 个提交（`is` 移除 `=>`／PA-13、`codegen.c` 死代码全链清理、xmake PATH 探测修复、BNF v2.1/v2.2 收敛、TODO 分层、文档-实现一致性核对）已就绪，门禁 c/native 各 12P/0F/5S；`main` 已重新对齐为 `PA` 镜像。tag 与推送由 ltree 决定（见 §1 发布门禁）。