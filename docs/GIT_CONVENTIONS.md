...
- **2026-09-19 `v1.0.2` 发布准备（待 tag）**：`PA` 上 v1.0.1 之后的全部提交（`is` 移除 `=>`／PA-13、`is _` 通配符补全／PA-15、`codegen.c` 死代码全链清理、xmake PATH 探测修复、BNF v2.1/v2.2 收敛、TODO 分层、文档-实现一致性核对、禁止自动推送规则入文档、共享文档 PA↔PB 双向同步）已就绪，门禁 c/native 各 12P/0F/5S；`main` 已重新对齐为 `PA` 镜像。tag 与推送由 ltree 决定（见 §1 发布门禁）。
