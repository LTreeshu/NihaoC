...
- [ ] **P1 全量 parser（parser.c）缺口：关键字内置函数未处理**——TOK_VISOF/TOK_SIZEOF/TOK_TYPEOF/TOK_ALIGNOF/TOK_OFFSETOF 均无对应解析（parser.c 0 处），这些是关键字 token，永远走不到 parser.c 的 identifier 字符串比较分支（该分支只能命中普通标识符 malloc）→ 全量 c/native 后端不支持 visof/sizeof/typeof/alignof/offsetof。修法：在 parse_primary 加 token case 或把这些移出关键字表
