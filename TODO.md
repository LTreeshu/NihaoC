...
- [ ] **P1 全量 parser（parser.c）缺口：visof 关键字未处理**——TOK_VISOF 无对应解析（parser.c 0 处），visof 是关键字 token 不会走 identifier 字符串比较分支 → 全量 c/native 后端不支持 visof
- [ ] **PB-6 内置函数**：malloc / sizeof / typeof / offsetof / structof 等（sizeof/malloc 等是关键字 token）
