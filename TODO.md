...
- [ ] **PB-8 多返回值、函数指针、多变量声明**：多变量声明 `var {a=0,b=1} i8` **已完成（2026-08-06）**——ir_stmt 前缀后 `{` 走 ir_multi_decl（收集 name=init 对 → 类型/聚合/数组 → 逐个 var_declare+MOV），无前缀 `{` 仍是块语句（无冲突）；用例 ir_multi.nc（IR_ONLY）双后端通过。**顺带补齐 &&/|| 短路逻辑层**（ir_logical_and/or，JZ/JNZ 跳转跳过右侧求值）。**函数指针最小集已完成（2026-08-06）**——新指令 `IR_CALLI`（dst=call *(a)）：ir_to_c 生成 `((int64_t (*)())tN)(args)`，ir_to_native 生成 `movq slot(a),%rax; call *%rax`；声明 `fp void(i32,i32) i32 = add2`（声明分支跳过函数指针类型参数列表+返回类型）；函数名引用 → IR_LD_ADDR sym（取函数地址）；变量后跟 `(` → 间接调用。用例 ir_fptr.nc（IR_ONLY）双后端通过。**剩余：多返回值（multireturn，需 IR_CALL 多返回，如 rax+rdx 或结构体）**
