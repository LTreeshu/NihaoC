...
- [ ] **PB-8 多返回值、函数指针、多变量声明**：多变量声明 `var {a=0,b=1} i8` **已完成（2026-08-06）**——ir_stmt 前缀后 `{` 走 ir_multi_decl（收集 name=init 对 → 类型/聚合/数组 → 逐个 var_declare+MOV），无前缀 `{` 仍是块语句（无冲突）；用例 ir_multi.nc（IR_ONLY）双后端通过。**顺带补齐 &&/|| 短路逻辑层**（ir_logical_and/or，JZ/JNZ 跳转跳过右侧求值）。剩余：函数指针（`void(u8) i32` 类型 + 间接调用，需 IR 结构扩展）、多返回值（multireturn，需 IR_CALL 多返回）
