...
- **PA TODO 处理完毕**：PA-1 ~ PA-32 全部 `[x]`；唯余 PA-33（`flow → flow` 转移因检查顺序无法书写）与 PA-34（多变量声明的数组后缀被丢弃）登记待决策，不纳入本版。
...
- **回归验证**：c / native 双后端各 **33P / 0F / 5S**、ir-c / ir-native 各 **6P / 0F / 28S**（0 FAIL，含 PA-18 ~ PA-22、PA-25 ~ PA-27、PA-31 十一条新 err 用例与 `ownerof` / `deref_slice` / `len_builtin` / `elseif_chain` / `alignof_builtin` / `slice_str_infer` / `print_forms` / `is_no_fallthrough` / `str_array_init` / `flow_no_free` 用例），跨后端一致性检查全部通过；examples 6/7 编译运行（`06_cooking` 为 2.0 预览，预期不通过，IR 后端下可通过）；`pattern.nc` 经 ir-c / ir-native 实测输出与全量后端一致。
