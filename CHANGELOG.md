...
- **PA TODO 处理完毕**：PA-1 ~ PA-15、PA-17 ~ PA-30 全部 `[x]`；唯余 PA-16（`is` 多子句 fallthrough 缺口）、PA-31（`T[n] x = "literal"` 声明容量丢失）、PA-32（`flow` 绑定字符串字面量 → `free` 静态存储）与 PA-33（`flow → flow` 转移因检查顺序无法书写）登记待决策，不纳入本版。
...
- **回归验证**：c / native 双后端各 **28P / 0F / 5S**、ir-c / ir-native 各 **6P / 0F / 23S**（0 FAIL，含 PA-18 ~ PA-22、PA-25 ~ PA-27 九条新 err 用例与 `ownerof` / `deref_slice` / `len_builtin` / `elseif_chain` / `alignof_builtin` / `slice_str_infer` / `print_forms` 用例），跨后端一致性检查全部通过；examples 6/7 编译运行（`06_cooking` 为 2.0 预览，预期不通过，IR 后端下可通过）；`pattern.nc` 经 ir-c / ir-native 实测输出与全量后端一致。
