...
- **`do` 体内 `is` 在 IR 前端拒绝（规范合规）**：IR 前端 `TOK_WHILE || TOK_DO` 分支此前无条件把条件值写入 `is_val_vreg`（守卫变量 `is_do` 算了却未使用），导致 `do cond { is pat { ... } }` 在 ir-c / ir-native 下被接受，与 BNF §6 / 中英 §6.1「`do` 不支持 `is`」相反（A 后端经 PA-18 已拒绝）。现 `do` 分支置 `is_val_vreg = -1`，体内 `is` 报 `ir: 'is' pattern match only valid inside while loop body`（文案去掉 `while/do` 的 `/do`），与 PB-27.1 对齐。新增 `tests/err/is_in_do_body.nc`。测试框架同步收紧：err 用例改为仅 `IR_ERR_SKIP` 列出的 M2 四条（`m2a`~`m2d`）在 IR 后端跳过，其余 err 用例四后端均执行。（PA-19）
