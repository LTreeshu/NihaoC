...
1.0 冻结线的规范合规与代码卫生版本，无新增语言特性（`is _` 通配符属补全文档已承诺的既有语法，非新特性）；三处语法收敛为**移除冗余记号**——`is` 的 `=>` 单语句形式（PA-13）、`?=` 安全赋值（PA-20）、`?.` / `?(` 安全解引用（PA-21），检查语义统一由 `=` 与 `.()` 自身承担。
...
- **PA TODO 处理完毕**：PA-1 ~ PA-15、PA-17 ~ PA-21 全部 `[x]`；唯余 PA-16（上述 fallthrough 缺口）登记待决策，不纳入本版。
...
- **回归验证**：c / native 双后端各 **17P / 0F / 5S**、ir-c / ir-native 各 **5P / 0F / 13S**（0 FAIL，含 PA-18 ~ PA-21 五条新 err 用例），跨后端一致性检查全部通过；examples 6/7 编译运行（`06_cooking` 为 2.0 预览，预期不通过）；`pattern.nc` 经 ir-c / ir-native 实测输出与全量后端一致。
