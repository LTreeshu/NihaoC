...
-- IR_ERR_SKIP: IR 前端未实现 M2 静态检查、不支持 `.(T)` 类型化解引用，也不支持从属/位域内置函数，
-- 这些 err 用例对其无意义（其余 err 用例双前端都跑）
local IR_ERR_SKIP = {m2a_flow_static = true, m2b_const_flow = true, m2c_frozen = true, m2d_invalid = true, deref_bounds = true, structof_bad_member = true}
